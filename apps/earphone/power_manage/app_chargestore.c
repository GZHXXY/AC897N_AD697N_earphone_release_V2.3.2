...
static void bt_get_tws_local_addr_from_vm(u8 *addr)
{
...
bool chargestore_set_tws_remote_info(u8 *data, u8 len)
{
    ...
    bt_get_tws_local_addr_from_vm(local_addr);
    ...
    if (__this->testbox_status && w_ret) {
        log_error("w_ret:%d\n", w_ret);
        //注意该vm id不要和syscfg_id.h中的定义重复
#define     VM_TESTBOX_PAIR_ERR			199
        syscfg_write(VM_TESTBOX_PAIR_ERR, &w_ret, sizeof(w_ret));
    }
...
u16 chargestore_get_tws_remote_info(u8 *data)
{
    ...
    CHARGE_STORE_INFO *charge_store_info = (CHARGE_STORE_INFO *)data;
#if TCFG_USER_TWS_ENABLE
    bt_get_tws_local_addr_from_vm(charge_store_info->tws_local_addr);
...
void app_chargestore_testbox_sub_cmd_handle(u8 *buf, u8 len)
{
    ...
    case CMD_BOX_ENTER_STORAGE_MODE:
        ...
        ex_enter_storage_mode_flag = 1;//断开测试盒生效
        chargestore_api_write(send_buf, 2);
        break;
