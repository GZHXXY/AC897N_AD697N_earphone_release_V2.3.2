...
u8 key_table[KEY_NUM_MAX][KEY_EVENT_MAX] = {
    ...
    {KEY_MUSIC_PP,   KEY_ANC_SWITCH,  KEY_NULL,  KEY_NULL,     KEY_NULL,     KEY_MUSIC_NEXT},   //KEY_0
...
/*MIC LDO电压档位设置,也会影响MIC的偏置电压0:1.5v  1:1.8v  2:2.1v  3:2.4v 4:2.7v 5:3.0 */
	.mic_ldo_vsel  = 3,
...
#if (TCFG_AUDIO_MIC_PWR_CTL && (TCFG_AUDIO_MIC_PWR_PORT != NO_CONFIG_PORT))
	/*使能mic电源管理的时候，不需要内部输出偏置*/
	.mic_bias_inside = 0,
#else
	/*MIC电容隔直模式使用内部mic偏置(PA2)*/
	.mic_bias_inside = 1,
...
/*MIC1内部上拉电阻挡位配置，影响MIC的偏置电压
	...
	10:3.91K  	9:4.41K 	8:5.0K  	7:5.6K		6:6K		5:6.5K		4:7K		3:7.6K		2:8.0K		1:8.5K				*/
    .mic1_bias_res   = 18,
#if (TCFG_AUDIO_MIC_PWR_CTL && (TCFG_AUDIO_MIC1_PWR_PORT != NO_CONFIG_PORT))
	/*使能mic电源管理的时候，不需要内部输出偏置*/
	.mic1_bias_inside = 0,
#else
...
#if TCFG_CHARGE_ENABLE || TCFG_CHARGESTORE_ENABLE || TCFG_TEST_BOX_ENABLE || TCFG_ANC_BOX_ENABLE
...
void board_init()
{
...
#if (TCFG_LP_TOUCH_KEY_ENABLE == DISABLE_THIS_MOUDLE)
	//V_SENSOR
	JL_PORTC->PD  &= ~BIT(0);
	JL_PORTC->PU  &= ~BIT(0);
	JL_PORTC->OUT |= BIT(0);
	...
	audio_mic_pwr_ctl(MIC_PWR_INIT);
...
void board_set_soft_poweroff(void)
{
    ...
    //MIC脚开下拉,把隔直电容变成对地电容,过滤LDOIN毛刺
    gpio_set_pull_down(IO_PORTB_08, 1);
...
static void port_edge_wakeup_callback(u8 index, u8 gpio)
{
#if TCFG_EAR_DETECT_ENABLE
#if (TCFG_EAR_DETECT_TYPE == EAR_DETECT_BY_TOUCH) && (!TCFG_EAR_DETECT_TOUCH_MODE)
...
void board_power_init(void)
{
	...
	port_edge_wkup_set_callback(port_edge_wakeup_callback);
#endif
#endif
...
#endif /* #ifdef CONFIG_BOARD_AC8976A_ANC */
