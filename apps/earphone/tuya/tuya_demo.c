...
{
    printf("tuya_app_send_user_data");
    int ret = 0;
...
void tuya_one_click_connect_init()
{
    ...
    bt_state_info.dev_info = tuya_ble_current_para.sys_settings.bound_flag;
    bt_state_info.connect_info = get_curr_channel_state();
    bt_state_info.name_len = strlen(bt_get_local_name());
    memcpy(bt_state_info.name, bt_get_local_name(), LOCAL_NAME_LEN);
