...
static int audio_fade_timer_add(u8 gain_l, u8 gain_r)
{
    ...
    /*防止联合音量还在淡入淡出的时候，数字音量无法淡入淡出*/
    if (__this->fade_timer) {
        usr_timer_del(__this->fade_timer);
        __this->fade_timer = 0;
    }
...
static void audio_combined_fade_timer(void *priv)
{
    ...
    if ((gain_l == __this->fade_gain_l) \
        ...
        local_irq_enable();
        audio_dac_set_analog_vol(&dac_hdl, __this->fade_gain_l);
...
#define TRIM_VALUE_LR_DIFF_ERR_MAX      (100)   // 左右声道的差值限制
...
int audio_dac_trim_value_check(struct audio_dac_trim *dac_trim)
{
    ...
    s16 reference = 0;
    if (((get_chip_version() & 0xff) >= 0x03) && ((get_chip_version() & 0xff) < 0x0C)) { //D版芯片及以后
        reference = -1200;
    } else {
        reference = 0;
    }
    ...
    if ((TCFG_AUDIO_DAC_CONNECT_MODE == DAC_OUTPUT_LR) \
        || (TCFG_AUDIO_DAC_CONNECT_MODE == DAC_OUTPUT_MONO_LR_DIFF)) {
        if (abs(dac_trim->left - dac_trim->right) > TRIM_VALUE_LR_DIFF_ERR_MAX) {
            log_error("dac trim channel diff err\n");
            return -1;
        }
    }
...
void dac_analog_open_cb(struct audio_dac_hdl *dac)
{
    printf(">>> dac_analog_open_cb !!!\n\n");
    delay(1000);
    SFR(JL_ADDA->DAA_CON2,  7,  1,  1);         // TRIM_EN
    SFR(JL_ADDA->DAA_CON0,  8,  1,  1);         // CHOPPER_CLK_EN
    SFR(JL_ADDA->DAA_CON0, 30,  1,  1);         // LPF_MILLER_EN
    SFR(JL_ADDA->DAA_CON2,  6,  1,  1);         // SDDAC_R_EN
    SFR(JL_ADDA->DAA_CON2,  5,  1,  1);         // SDDAC_L_EN
    SFR(JL_ADDA->DAA_CON2, 17,  1,  1);         // SDDAC_MODE_SEL
    SFR(JL_ADDA->DAA_CON0, 27,  3,  dac->pd->lpf_isel);         // LPF_ISEL[2:0]
    SFR(JL_ADDA->DAA_CON0, 24,  2,  dac->pd->ldo_isel);         // LDO_ISEL[1:0]
    SFR(JL_ADDA->DAA_CON0, 22,  2,  dac->pd->ldo_fb_isel);      // LDO_FB_ISEL[1:0]
    SFR(JL_ADDA->DAA_CON0, 14,  1,  1);         // DACVCM_EN
    SFR(JL_ADDA->DAA_CON0, 19,  1,  0);
    SFR(JL_ADDA->DAA_CON0, 15,  1,  0);         // DACVCM_RSEL
    SFR(JL_ADDA->DAA_CON0, 31,  1,  0);         // LPF_MUTE
    SFR(JL_ADDA->DAA_CON0, 26,  1,  dac->pd->zero_cross_detect);         // LPF_2_ZERO_EN
    if (dac->anc_toggle == 0) {
        SFR(JL_ADDA->DAA_CON1,  4,  4,  0);         // RG_SEL[3:0]
        SFR(JL_ADDA->DAA_CON1,  0,  4,  0);         // LG_SEL[3:0]
    }
    switch (dac->pd->output) {
    case DAC_OUTPUT_MONO_L:
        JL_ADDA->DAA_CON0 |= BIT(20);
        break;
    case DAC_OUTPUT_MONO_R:
        JL_ADDA->DAA_CON0 |= BIT(21);
        break;
    case DAC_OUTPUT_LR:
    case DAC_OUTPUT_MONO_LR_DIFF:
        JL_ADDA->DAA_CON0 |= BIT(20) | BIT(21);
        break;
    default:
        break;
    }
    SFR(JL_ADDA->DAA_CON2,  2,  3,  3);         // SDDAC_DTSEL_11v[2:0]
    SFR(JL_ADDA->DAA_CON0, 17,  2,  dac->pd->ldo_volt);         // DACVDD_LDO_VSEL_11v[1:0]
    SFR(JL_ADDA->DAA_CON0, 16,  1,  1);         // DACVDD_LDO_EN_11v
    delay(1000);
    SFR(JL_ADDA->DAA_CON2, 11,  1,  1);         // VCM_EN
    SFR(JL_ADDA->DAA_CON0,  7,  1,  1);         // VBIAS_EN
    SFR(JL_ADDA->DAA_CON0,  6,  1,  1);         // IBIAS_EN
    os_time_dly(2);
    SFR(JL_ADDA->DAA_CON0,  5,  1,  0);
    SFR(JL_ADDA->DAA_CON0,  4,  1,  0);
    SFR(JL_ADDA->DAA_CON0,  3,  1,  0);
    SFR(JL_ADDA->DAA_CON0,  2,  1,  0);
    SFR(JL_ADDA->DAA_CON0,  1,  1,  0);
    SFR(JL_ADDA->DAA_CON0,  0,  1,  0);

    SFR(JL_ADDA->DAA_CON2, 31,  1,  dac->pd->zero_cross_detect);         // ZCEN
    SFR(JL_ADDA->DAA_CON2, 20,  1,  0);         // PNS_EN
    SFR(JL_ADDA->DAA_CON2, 19,  1,  0);         // PNS50K_EN
    SFR(JL_ADDA->DAA_CON2, 16,  1,  dac->pd->zero_cross_detect);         // ZERO_RC_EN
    SFR(JL_ADDA->DAA_CON2, 15,  1,  dac->pd->zero_cross_detect);         // ZERO_LC_EN
    SFR(JL_ADDA->DAA_CON2, 13,  2,  0);         // ZERO_DETECT_ISEL[1:0]
    SFR(JL_ADDA->DAA_CON2, 12,  1,  0);         // VCM_RISETIME_EN
    SFR(JL_ADDA->DAA_CON2, 10,  1,  0);         // TRIM_SW
    SFR(JL_ADDA->DAA_CON2,  8,  2,  0);         // TRIM_SEL[1:0]
    SFR(JL_ADDA->DAA_CON2,  0,  2,  0);         // LS_DAC_DTSEL[1:0]

    SFR(JL_ADDA->DAA_CON3,  4,  1,  0);
    SFR(JL_ADDA->DAA_CON3,  3,  1,  0);
    SFR(JL_ADDA->DAA_CON3,  2,  1,  0);
    SFR(JL_ADDA->DAA_CON3,  1,  1,  0);
    SFR(JL_ADDA->DAA_CON3,  0,  1,  0);

    SFR(JL_ADDA->DAA_CON2, 18,  1,  0);         // PNS10K_EN
    SFR(JL_ADDA->DAA_CON2,  7,  1,  0);         // TRIM_EN
}

void dac_analog_close_cb(struct audio_dac_hdl *dac)
{
    printf(">>> dac_analog_close_cb !!!\n\n");
    extern int audio_mic_ldo_state_check(void);
    u8 mic_ldo_keep = audio_mic_ldo_state_check();
    JL_ADDA->DAA_CON2 |= BIT(7); //trim enable

    SFR(JL_ADDA->DAA_CON2, 19,  1,  1);         // PNS10K_EN
    SFR(JL_ADDA->DAA_CON2,  6,  1,  0);         // SDDAC_R_EN
    SFR(JL_ADDA->DAA_CON2,  5,  1,  0);         // SDDAC_L_EN
    SFR(JL_ADDA->DAA_CON2, 17,  1,  0);         // SDDAC_MODE_SEL
    JL_ADDA->DAA_CON0 &= ~(BIT(20) | BIT(21)); //HP EN L R
    if (mic_ldo_keep == 0) {
        JL_ADDA->DAA_CON0 &= ~BIT(6) ;   //vbias dis ibias dis

        JL_ADDA->DAA_CON2 &= ~BIT(11); //vcm dis
    }
    JL_ADDA->DAA_CON0 &= ~(BIT(14) | BIT(16));  //dacvcm dis ldo dis analog dis
    delay(3000);
    if (mic_ldo_keep == 0) {
        JL_ADDA->DAA_CON0 &= ~BIT(7);   //audio vbias dis audio ibias dis
    }
    delay(2000);
    SFR(JL_ADDA->DAA_CON2, 19,  1,  0);         // PNS10K_EN
    delay(1000);
    JL_ADDA->DAA_CON2 &= ~BIT(7); //trim disable
}
