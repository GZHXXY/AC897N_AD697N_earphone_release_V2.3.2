...
/*用于同步主从机的设备型号参数*/
#define TWS_FUNC_ID_PHONE_MANUFACTURER    TWS_FUNC_ID('P', 'M', 'A', 'R')
static void bt_tws_anc_phone_manufacturer(void *_data, u16 len, bool rx)
{
    if (rx) {
        u8 *data = (u8 *)_data;
        /* r_printf("[slave]phone_manufacturer: %d, %d\n", data[0], data[1]); */
        app_var.remote_dev_company = data[0];
    }
}

REGISTER_TWS_FUNC_STUB(app_anc_phone_manufacturer) = {
    .func_id = TWS_FUNC_ID_PHONE_MANUFACTURER,
    .func    = bt_tws_anc_phone_manufacturer,
};

void bt_tws_sync_phone_manufacturer(void)
{
    u8 data[2];
    data[0] = app_var.remote_dev_company;
    data[1] = 0;
    /* r_printf("[master]phone_manufacturer: %d, %d\n", data[0], data[1]); */
    tws_api_send_data_to_slave(data, 2, TWS_FUNC_ID_PHONE_MANUFACTURER);
}
...
int bt_tws_connction_status_event_handler(struct bt_event *evt)
{
    ...
    case TWS_EVENT_CONNECTED:
        ...
        if (role == TWS_ROLE_MASTER) {
            bt_tws_sync_phone_manufacturer();
        }
