...
void volume_up(u8 inc)
{
    ...
    printf("cur_vol is:%d, max:%d\n", cur_vol, app_audio_get_max_volume());
    if ((get_bt_tws_connect_status() == 0) || (tws_api_get_role() == TWS_ROLE_MASTER)) {
        extern void tuya_valume_indicate(u8 valume);
        u8 tuya_sync_valume;
        if (cur_vol > 14) {
            tuya_valume_indicate(100);
        } else {
            if (cur_vol == 0) {
                cur_vol++;
            }
            tuya_sync_valume = (int)(cur_vol * 100 / max_vol);
...
#if TUYA_DEMO_EN
    s8 cur_vol = app_audio_get_volume(APP_AUDIO_STATE_MUSIC);
    u8 max_vol = app_audio_get_max_volume();
    printf("cur_vol is:%d, max:%d\n", cur_vol, app_audio_get_max_volume());
    if ((get_bt_tws_connect_status() == 0) || (tws_api_get_role() == TWS_ROLE_MASTER)) {
        extern void tuya_valume_indicate(u8 valume);
...
int app_earphone_key_event_handler(struct sys_event *event)
{
    ...
    case  KEY_MUSIC_PP:
        ...
        tone_play_index(IDEX_TONE_RING, 0);
        mem_stats();
        break;
#endif
