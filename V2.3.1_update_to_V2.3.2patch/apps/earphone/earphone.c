...
static int bt_connction_status_event_handler(struct bt_event *bt)
{
    ...
    case BT_STATUS_INIT_OK:
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 6; j++) {
                printf("key_table_l:%d, key_table_r:%d\n", key_table_l[i][j], key_table_r[i][j]);
            }
        }
#endif
...
#if TUYA_DEMO_EN
        extern void tuya_play_status_indicate(u8 status);
...
extern void bt_vendor_test_mode_event_handle(u8 *arg, u8 len);
static void bt_vendor_meta_event_handle(u8 sub_evt, u8 *arg, u8 len)
{
    ...
    if (sub_evt == HCI_SUBEVENT_VENDOR_TEST_MODE_CFG) {
        bt_vendor_test_mode_event_handle(arg, len);
    } else {
...
static int bt_hci_event_handler(struct bt_event *bt)
{
    ...
    case HCI_EVENT_CONNECTION_COMPLETE:
        ...
        case ERROR_CODE_SUCCESS :
            ...
            extern void testbox_in_ear_detect_test_flag_set(u8 flag);
...
#if TME_EN
extern void TME_msg_deal(void *parm);
#endif
/*
 * 系统事件处理函数
 */
...
static int state_machine(struct application *app, enum app_state state, struct intent *it)
{
    ...
    case APP_STA_DESTROY:
        ...
        sys_enter_soft_poweroff((void *)4);
        break;
    }
