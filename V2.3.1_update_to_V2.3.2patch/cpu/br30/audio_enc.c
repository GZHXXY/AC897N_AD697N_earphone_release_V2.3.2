...
static void audio_iis_mic_output(s16 *data, u32 len)
{
    ...
    u32 i;
    if (esco_enc) {
        local_bt_us_time(&esco_enc->cur_clkn, &esco_enc->cur_finecnt);
        u32 time_gap = ((esco_enc->cur_clkn - esco_enc->pre_clkn) & 0x7ffffff) * 625 + (esco_enc->cur_finecnt - esco_enc->pre_finecnt);
        if (esco_enc->cur_clkn) {
            esco_enc->in_sample_rate  = 1000000 / (time_gap / ALNK_BUF_POINTS_NUM);
        }
        esco_enc->pre_clkn = esco_enc->cur_clkn;
        esco_enc->pre_finecnt = esco_enc->cur_finecnt;
...
#else/*master mode*/
    esco_enc->in_sample_rate =  625 * TCFG_IIS_SAMPLE_RATE / 624;
    esco_enc->out_sample_rate = fmt.sample_rate;//624 * 20;
#endif/*TCFG_IIS_MODE*/
